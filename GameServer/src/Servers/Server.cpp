...
void Server::Start() {
    ...
    pool =new ThreadPool(6);//创建线程池数量为6
	socket_m.server = this;
	socket_m.SocketInit();//初始化Socket
}
//将连接的客户端保存，及加入线程池进行数据收发
void Server::AddClient(int  c, LPSOCKET_INFORMATION SocketInfo, unsigned long i) {
...
void Server::RemoveSocket(int  c) {
